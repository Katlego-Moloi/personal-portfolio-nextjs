...
export const Certifications = () => {
  return(
    <section className="py-20 w-full" id="certs">
      ...
      <div className="no-scrollbar relative flex w-full overflow-hidden mt-12">
        {/* Fade masks */}
        ...
        <div className="flex w-max animate-move-left gap-8 pr-8">
          {certs.map((cert) => (
            <a href={cert.link} target="_blank" rel="noopener noreferrer">
              <div className="card items-center justify-center flex-shrink-0 w-50">
        ...
        <div className="flex w-max animate-move-left gap-8 pr-8">
          {certs.map((cert) => (
            <a href={cert.link} target="_blank" rel="noopener noreferrer">
              <div className="card items-center flex-shrink-0 w-50">
                <img src={cert.img} alt={cert.label} className="rounded-xl flex"/>
                <p className="text-sm text-center font-bold mt-7">{cert.label}</p>
              </div>
            </a>
